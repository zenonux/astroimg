export default function withProj(L, proj4) {
  L.Proj = {}
  ...
  L.Proj.Projection = L.Class.extend({
    initialize(code, def, bounds) {
      let isP4 = L.Proj._isProj4Obj(code)
      this._proj = isP4 ? code : this._projFromCodeDef(code, def)
      this.bounds = isP4 ? def : bounds
    },

    project(latlng) {
      let point = this._proj.forward([latlng.lng, latlng.lat])
      return new L.Point(point[0], point[1])
    },

    unproject(point, unbounded) {
      let point2 = this._proj.inverse([point.x, point.y])
      ...
      if (geojson) {
        ...
        if (crs !== undefined) {
          this.options.coordsToLatLng = function (coords) {
            let point = L.point(coords[0], coords[1])
  ...
  L.Proj.ImageOverlay = L.ImageOverlay.extend({
    ...
    _animateZoom(event) {
      ...
      let northWest = L.point(this._projectedBounds.min.x, this._projectedBounds.max.y)
      let offset = this._projectedToNewLayerPoint(northWest, event.zoom, event.center)
    ...
    _projectedToNewLayerPoint(point, zoom, center) {
      let viewHalf = this._map.getSize()._divideBy(2)
      let newTopLeft = this._map.project(center, zoom)._subtract(viewHalf)._round()
      let topLeft = newTopLeft.add(this._map._getMapPanePos())

      return this._transform(point, zoom)._subtract(topLeft)
    },

    _transform(point, zoom) {
      let crs = this._map.options.crs
      let transformation = crs.transformation
